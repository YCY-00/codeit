...
app.get("/api/members", async (request, response) => {
  ...
  if (team) {
    ...
    response.send(
      `<h1>${team} team's member list</h1>${JSON.stringify(teamMembers)}`
    );
...
app.get("/api/members/:id", async (request, response) => {
  ...
  if (member) {
    response.send(
      `<h1>The ${id}th member's info</h1><h2>${JSON.stringify(member)}</h2>`
    );
  } else {
    // undefined
...
app.post("/api/members", async (request, response) => {
  ...
  if (member) {
    ...
    response.send(
      `<h1>${newMember.id}th member has arrived</h1><h2>${JSON.stringify(
        request.body
      )}</h2>`
    );
  }
